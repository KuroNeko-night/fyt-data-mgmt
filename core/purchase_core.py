...
表头行与列位置自动识别，不写死列号。运行于 Windows 10/11 + Python 3.13。
"""
import os
import re
...
def norm_no(v):
    ...
    if isinstance(v, float) and v.is_integer():
        v = int(v)
    s = _common.clean_str(v).upper()           # 全角→半角、去零宽/NBSP,再大写
...
def load_rows(path, sheet=None, log=None):
    ...
    try:
        if sheet:
            ...
            # 未指定子表: 默认第一张; 若其表头认不出且存在多子表(封面/说明页占了
            # sheet0 的常见情形), 再扫其余子表取第一个能认出表头的, 不静默选错。
...
def match_rows(rows1, rows2):
    """一对一最优配对。返回 (matched1, matched2, pairs)。
    matched1/2: 布尔列表；pairs: [(i, j, score)]。

    先按名称和规格分桶缩小候选集，再用最小费用最大流求解：优先让配对数量
    最大，其次让编号/批次评分总和最大，避免贪心先占用“唯一对家”造成漏配。
    """
    m1 = [False] * len(rows1)
    m2 = [False] * len(rows2)
    pairs = []
...
def _optimal_bucket_pairs(rows1, rows2, left, right):
    ...
    def add_edge(start, end, capacity, cost, meta=None):
        graph[start].append([end, len(graph[end]), capacity, cost, meta])
    ...
    for rj in range(n_right):
        add_edge(right_base + rj, sink, 1, 0)
    for li, i in enumerate(left):
        for rj, j in enumerate(right):
            if _can_match(rows1[i], rows2[j]):
                score = _pair_score(rows1[i], rows2[j])
                # 评分权重大于所有顺序微调；后半只保证同分时结果稳定。
                cost = -score * 100000 + li * max(1, n_right) + rj
                add_edge(left_base + li, right_base + rj, 1, cost,
                         (i, j, score))
    ...
    result = []
    for li in range(n_left):
        for edge in graph[left_base + li]:
...
def run(file1, file2, sheet1=None, sheet2=None, name1="我方", name2="供方",
    ...
    # 四段:读两表 30% / 配对+疑点 20% / 上色两表(写盘) 35% / 汇报单 15%。
