...
仅用 pypdf + openpyxl，运行于 Windows 10/11 + Python 3.13。
...
def scan(root, log=None, progress=None):
    ...
    progress : 可选进度回调(0~100)；Worker 按 scan 是否有此形参自动注入。"""
    def _lg(m):
        if log:
            log(m)
    ...
    for _pi, p in enumerate(pdfs):
        ...
        for inv in invs:
            ...
            if inv.num in by_num:
                ...
                continue
            by_num[inv.num] = inv
        prog.tick(_pi + 1, n_pdf)         # 每识别完一个 PDF 推进一格
...
def generate(result, rows, ym, out_dir=None, log=None, progress=None):
    ...
    progress: 可选进度回调(0~100)；Worker 按 generate 是否有此形参自动注入。
    """
    def _lg(msg):
        if log:
            log(msg)
    # 两段:写汇总表 50% / 导出专票复核文件夹(拷 PDF,较重) 50%。
    ...
    xlsx = _common.unique_path(os.path.join(out_dir, "%s统计增值税发票.xlsx" % mm))  # 同月重跑不覆盖
