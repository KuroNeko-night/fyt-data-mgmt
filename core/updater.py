...
仅用标准库(urllib)，不引入额外依赖，运行于 Windows 10/11 + Python 3.13。
"""
import json
import ssl
import hashlib
...
def check_update(timeout=8):
    ...
    # 清单决定安装包地址与哈希，不能经第三方代理取得；代理可同时篡改两者。
    try:
...
def download_installer(url, dest_dir=None, progress=None, log=None, timeout=30,
    ...
    try:
        ...
        # 网络连接在创建响应前失败时也要清理旧半包，避免下次启动误认为可续传。
        try:
            if os.path.exists(part):
                os.remove(part)
        except OSError:
