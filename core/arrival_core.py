...
FIRST_COL, BATCH_STRIDE, DATA_START_ROW = 3, 8, 8
DEFAULT_TOTAL = 566
DEFAULT_TOP_LABEL = "截止16点的数据"
COL_WIDTHS = [7.9, 17.6, 29.6, 34.4, 9.0, 11.9]
...
def extract_unreceived(path):
    """提取未收货物料: [编码, 名称, 供应商, 需求数].
       规则:
       1) 跳过被筛选隐藏的行(尊重用户在 Excel 里的筛选);
       2) "剩余未收数"列为 #N/A / #REF! 等错误值时, 一律当作 0(已收)处理, 不计入;
       3) 只保留"剩余未收数"为非零数值的行(0、空、错误值都不算未收)。"""
    import numbers
    ...
    for r in range(3, ws.max_row + 1):
        ...
        if code is None:
            continue
        remain = ws.cell(row=r, column=COL_REMAIN).value
        if isinstance(remain, bool):
            continue
        # #N/A 等错误(字符串) / None / 0 -> 当作已收, 排除; 只认非零数值
        if not isinstance(remain, numbers.Number) or remain == 0:
            continue
        rows.append([str(code), ws.cell(row=r, column=COL_NAME).value,
                     ws.cell(row=r, column=COL_SUPPLIER).value,
                     ws.cell(row=r, column=COL_DEMAND).value])
    return rows
...
def _write_batch(ws, c0, batch_no, materials, total, remark, top_label):
    ...
    # 第6行: 备注(有则写, 跨6列, 左对齐, 白底)
    if remark:
        ws.cell(row=6, column=c0, value="备注: " + str(remark))
        ws.merge_cells(start_row=6, start_column=c0, end_row=6, end_column=c0 + 5)
        for cc in range(c0, c0 + 6):
            _style(ws.cell(row=6, column=cc), align=LEFT)
    # 第7行: 表头(蓝底加粗)
    for i, h in enumerate(["序号", "物料编码", "物料名称", "供应商信息", "需求数", "备注"]):
        _style(ws.cell(row=7, column=c0 + i, value=h), bold=True, fill=True)
    # 数据行(白底)
    for i, (code, name, supp, dem) in enumerate(materials):
        r = DATA_START_ROW + i
        for j, v in enumerate([i + 1, code, name, supp, dem, None]):
