...
export function MobileNavigation({ activeKey, user, unreadCount, pendingReviews, onNavigate, onOpenMore }: Props) {
  ...
  const items = NAVIGATION_ITEMS.filter((item) => item.mobilePrimary && isNavigationAllowed(item, user.role));  // 同时应用主入口标记和角色权限，保证按钮数量稳定且不越权展示
  // “更多”徽标合并消息与待确认复核数，保持入口汇总口径与桌面侧栏一致。
  return <nav className="fyt-mobile-nav" aria-label="移动端主导航">
    {items.map((item) => {
      // 任一具体业务工作区都归属于“业务模块”主入口。
      const active = item.key === "features" ? activeKey === "features" || activeKey.startsWith("feature:") : activeKey === item.key;  // 任一具体业务工作区都归属“业务模块”入口
