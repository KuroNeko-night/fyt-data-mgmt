...
export const BUSINESS_GUIDANCE: Record<string, BusinessGuidance> = {
  ...
  invoice: { key: "invoice", purpose: "识别 PDF 发票并按月份整理台账。", template: { name: "月度 PDF 发票", description: "同一统计月份的多张供应商发票。", headers: ["发票号码", "供应商", "开票日期", "不含税金额", "价税合计"], rows: [row("INV-001", "供应商 A", "2026-08-18", "1000", "1130"), row("INV-002", "供应商 B", "2026-08-19", "800", "904")], output: "发票台账：发票号码、供应商、税额和月份汇总。", tips: ["上传前按月份归集发票更容易复核。", "识别不确定的发票会进入人工确认。"] } },
...
export const WORKFLOW_DEFINITIONS: WorkflowDefinition[] = [
  ...
  { key: "finance", title: "财务票货核对", description: "先识别发票，再与采购明细比对供应商票货关系。", audience: "适合财务人员", steps: [
    { featureKey: "invoice", title: "生成发票台账", description: "按月份识别 PDF 发票并汇总。", input: "月度 PDF 发票" },
