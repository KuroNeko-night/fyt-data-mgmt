...
def build_daily_report_snapshot(
    ...
    with deps.db_lock, deps.db() as connection:
        arrival_rows = connection.execute(  # 任务历史跨账号聚合，仅管理员可进入本函数。
        ...
        attendance_rows = deps.attendance_rows(connection, report_date)
        production_attendance_rows = deps.production_attendance_rows(connection, report_date)
        ...
        monthly_plan_rows = connection.execute(  # 当月计划可能由不同日期多次上传，Core 决定如何汇总。
            "SELECT p.*, COALESCE(u.display_name, u.username, '') AS uploaded_by_name "
            "FROM daily_production_plans p LEFT JOIN users u ON u.id = p.uploaded_by "
    ...
    for row in arrival_rows:
        try:
            ...
            result = {}  # 历史坏记录不能拖垮整张管理看板，保留任务元数据供排查。
        arrival_jobs.append({
            "id": row["id"],
            "title": row["title"],
    ...
    source_uploads = [deps.daily_source_upload_public(row) for row in source_rows]
    ...
    images_by_issue: dict[str, list[sqlite3.Row]] = {}  # 一次查询后分组，避免每条问题追加一次数据库往返。
    ...
    attendance_people = [deps.daily_attendance_public(row) for row in attendance_rows]
    production_groups = [deps.daily_production_attendance_public(row) for row in production_attendance_rows]
    ...
    return deps.build_snapshot(  # 所有展示指标和导出数据都从同一 Core 快照派生。
        ...
        brief_items=[deps.daily_brief_public(row) for row in brief_rows],
        production_plans=[deps.production_plan_public(row) for row in plan_rows],
        monthly_production_plans=[deps.production_plan_public(row) for row in monthly_plan_rows],
