...
class AttendanceSourceTests(unittest.TestCase):
    ...
    def test_manual_mapping_precedes_header_guess(self):
        ...
        _write_source(source, ["人员", "业务日", "首次", "末次"], [["张三", "2026-08-11", "08:01", "17:02"]])
        options = Options(columns={
            os.path.basename(source): {
                "sheet": "每日统计",
                "header": 1,
                "roles": {"name": 0, "date": 1, "on": 2, "off": 3},
            },
        })
        records = attendance_core.load_source(source, options)
        self.assertEqual(records[("张三", (2026, 8, 11))], ("08:01", "17:02"))
