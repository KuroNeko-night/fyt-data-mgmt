...
class SupplierBatchCoreTests(unittest.TestCase):
    ...
    def _write_batch(path, rows):
        ...
        worksheet.title = "汇总"
        worksheet.append([None] * 6)  # 前导空行模拟模板
    ...
    def test_analyze_scans_suppliers_and_excludes_original_items(self):
        ...
        self.assertEqual(counts, {"供应商乙": 2, "供应商丙": 1})  # 有效供应商行数
        self.assertEqual(plan["excluded_original_count"], 2)  # 原厂排除两行
        self.assertEqual(plan["unmatched_count"], 0)  # 无未匹配
        self.assertGreaterEqual(plan["supplier_conflicts"], 1)  # 同料多供应商冲突
        self.assertEqual([item["batch"] for item in plan["batches"]], ["GMIDR26178A", "GMIDR26178B"])  # 批次顺序
    ...
    def test_run_generates_only_selected_supplier_without_original_items(self):
        ...
        self.assertEqual(result["generated"], 1)  # 生成一个供应商文件
        self.assertEqual(result["rows"], 1)  # 仅选中供应商一行
        self.assertEqual(result["excluded_original_count"], 2)  # 原厂仍排除
        self.assertEqual(result["batch_dates"], {"GMIDR26178A": "8.7", "GMIDR26178B": "8.10"})  # 交付日期透传
        self.assertEqual(len(result["files"]), 1)
        self.assertIn("供应商丙", os.path.basename(result["files"][0]))  # 文件名含供应商
        ...
        try:
            ...
            workbook.close()
        self.assertIn("JBC003", values)  # 选中物料存在
        self.assertIn("GMIDR26178A交付日期8.7", values)  # 交付日期写入
        self.assertFalse(any("原厂" in str(value) for value in values))  # 原厂物料不输出
