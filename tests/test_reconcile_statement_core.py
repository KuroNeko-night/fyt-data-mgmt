...
class ReconcileStatementCoreTests(unittest.TestCase):
    ...
    def test_scan_detects_batches_and_yellow_rows(self):
        ...
        self.assertEqual(files["（已下单）甲供应商7月采购清单明细.xlsx"]["supplier"], "甲供应商")
        # 文件名识别不出时，从数据行供应商列兜底（众数）
        self.assertEqual(files["清单未命名.xlsx"]["supplier"], "客供件")
        batches = files["（已下单）甲供应商7月采购清单明细.xlsx"]["batches"]
        self.assertEqual([b["batch"] for b in batches], ["GKMYR26027-01", "GKMYR26027-02"])
        self.assertEqual(batches[0]["rows"], 3)
        self.assertEqual(batches[0]["excluded_rows"], 1)
    ...
    def test_build_groups_by_supplier_and_excludes_yellow(self):
        ...
        self.assertEqual(set(names), {"甲供应商202607月对单明细.xlsx", "客供件202607月对单明细.xlsx"})
        ...
        worksheet = workbook.active
        self.assertEqual(worksheet.cell(1, 1).value, "202607月甲供应商对账单")
        self.assertEqual([c.value for c in worksheet[2]], ["序号", "材料编号", "材料名称", "规格", "单位", "采购数量", "批次号", "备注"])
    ...
    def test_build_supplier_override_and_validation(self):
        ...
        self.assertEqual(result["files"][0]["name"], "丙供应商7月对单明细.xlsx")
        with self.assertRaises(ValueError):
            reconcile_statement_core.build([self.file_b], [], "202607", out_dir=out_dir)
        with self.assertRaises(ValueError):
            reconcile_statement_core.build([self.file_b], selected, "", out_dir=out_dir)
    ...
    def test_three_horizontal_blocks_and_second_row_are_all_detected(self):
        ...
        self.assertEqual(built["total_rows"], 1)
        workbook = openpyxl.load_workbook(built["files"][0]["path"], data_only=True)
        try:
            self.assertEqual(workbook.active.cell(3, 2).value, "JBC0101A0003")
            self.assertEqual(workbook.active.cell(3, 7).value, "GKMYR26027-03")
        finally:
            workbook.close()
    ...
    def test_supplier_override_cannot_escape_output_directory(self):
        ...
        self.assertEqual(os.path.commonpath([out_dir, target]), out_dir)
        self.assertNotIn("..", os.path.basename(target))
        self.assertEqual(result["out_dir"], out_dir)

    def test_supplier_from_filename(self):
        self.assertEqual(reconcile_statement_core.supplier_from_filename("（已下单）众瀚7月采购清单明细.xlsx"), "众瀚")
        self.assertEqual(reconcile_statement_core.supplier_from_filename("(已下单）圣直7月采购清单明细.xlsx"), "圣直")
        self.assertIsNone(reconcile_statement_core.supplier_from_filename("采购清单.xlsx"))
        self.assertIsNone(reconcile_statement_core.supplier_from_filename("（已下单）7月采购清单明细.xlsx"))
