...
class PivotConfidenceTests(unittest.TestCase):
    ...
    def test_complete_sources_keep_full_score(self):
        ...
        self.assertEqual(result, {"level": "可信", "score": 100, "issues": []})
    ...
    def test_all_penalties_preserve_order_and_threshold(self):
        ...
        self.assertIn("包装方案汇总", messages[0])
        self.assertIn("组托辅材", messages[1])
        self.assertIn("未识别.xlsx", messages[2])
        self.assertIn("缺失字段", messages[3])
        self.assertIn("低置信识别", messages[4])
        self.assertIn("透视总计为 0", messages[5])
        self.assertIn("分组数(3)大于清洗行数(2)", messages[6])
