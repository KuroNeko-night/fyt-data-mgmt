...
def _write_purchase(path, rows):
    ...
    for row in rows:
        worksheet.append([None, None] + list(row))
    workbook.save(path)
    workbook.close()
...
class InvoiceMatchCoreTests(unittest.TestCase):
    ...
    def test_match_classifies_suppliers(self):
        ...
        self.assertEqual(result["no_invoice"], 1)
        self.assertEqual(result["no_purchase"], 1)
        self.assertEqual(result["no_invoice_suppliers"], ["瑞敏"])
        self.assertEqual(result["no_purchase_suppliers"], ["独立新供应商"])
        self.assertTrue(os.path.isfile(result["path"]))

        workbook = openpyxl.load_workbook(result["path"], data_only=True)
        summary = workbook.active
        rows = [row for row in summary.iter_rows(min_row=2, values_only=True) if any(row)]
        by_supplier = {str(row[0]): row[3] for row in rows}
        self.assertEqual(by_supplier["客供件"], "正常")
        self.assertEqual(by_supplier["众瀚"], "正常")
        self.assertEqual(by_supplier["瑞敏"], "无票采购")
        self.assertEqual(by_supplier["独立新供应商"], "有发票无采购")
        self.assertEqual(by_supplier["客供件"], "正常")
        workbook.close()
    ...
    def test_missing_inputs_raise(self):
        ...
        with self.assertRaises(ValueError):
            invoice_match_core.match([], [self.purchase])
        with self.assertRaises(ValueError):
            invoice_match_core.match([self.invoice], [])
