...
class MasterDataImportCoreTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.environ["FYT_CATALOG_PATH"] = self.catalog_path
    ...
    def test_clean_batch_requires_confirmation_before_merge(self):
        ...
        self.assertEqual(material_catalog.load()["materials"], {})
        ...
        self.assertEqual(catalog["materials"]["M001"]["spec"], "100x50")
        self.assertTrue(merged["merge_summary"]["backup_path"])
    ...
    def test_in_file_conflict_keeps_sources_and_can_use_candidate(self):
        ...
        self.assertEqual({item["value"] for item in conflict["values"]}, {"铁箱", "铁箱新版"})
        self.assertEqual({source["row"] for item in conflict["values"] for source in item["sources"]}, {2, 3})
        ...
        self.assertEqual(reviewed["status"], "ready_to_confirm")
        master_data_import_core.confirm_batch(
            batch["id"], actor_id=1, actor_name="系统管理员", root=self.import_root)
        master_data_import_core.merge_batch(batch["id"], root=self.import_root)
        self.assertEqual(material_catalog.load()["materials"]["M001"]["name"], "铁箱新版")
    ...
    def test_catalog_conflict_can_keep_current(self):
        ...
        batch = self.analyze(path)
        conflict = next(item for item in batch["candidates"] if item["relation_type"] == "material_name")
        self.assertEqual(conflict["current_value"], "正式名称")
        ...
        master_data_import_core.confirm_batch(
            batch["id"], actor_id=1, actor_name="系统管理员", root=self.import_root)
        master_data_import_core.merge_batch(batch["id"], root=self.import_root)
        self.assertEqual(material_catalog.load()["materials"]["M001"]["name"], "正式名称")
    ...
    def test_new_conflict_after_confirmation_returns_to_review(self):
        ...
        self.assertEqual(conflict["current_value"], "其他任务写入")
        self.assertIsNone(conflict["decision"])
    ...
    def test_duplicate_file_is_rejected(self):
        ...
        with self.assertRaises(master_data_import_core.DuplicateImportError) as caught:
            self.analyze(path)
        self.assertEqual(caught.exception.batch_id, first["id"])
    ...
    def test_unrecognized_workbook_is_not_persisted(self):
        ...
        with self.assertRaisesRegex(ValueError, "没有识别到"):
            self.analyze(path)
        self.assertEqual(master_data_import_core.list_batches(self.import_root)["items"], [])
