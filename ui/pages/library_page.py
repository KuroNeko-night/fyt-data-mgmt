...
class LibraryPage(BasePage):
    ...
    def guide_steps(self):
        return [
            (None, "欢迎使用数据库",
             "这是程序自带的表格仓库:把各处的表拖进来,程序自动识别用途并归档。\n"
             "各功能页(考勤、对账、透视等)都能直接从这里取用所需的表,不必每次重新找文件。"),
            (self.drop, "① 拖入表格归档",
             "把任意表格拖到这里。程序自动识别它的用途(考勤来源、对账总表、采购数据…)\n"
             "并归到对应类别;认不准的放进「未识别」,可稍后手动改判。"),
            (self.tree, "② 看归档 · 可信度",
             "所有表按类别分组列在这里,显示最后更新、大小与识别可信度(绿高/橙中/红低)。\n"
             "鼠标悬停可看识别依据与归档路径。"),
            (self.search, "③ 搜索",
             "表多了就在这搜:表名、类别、来源都能匹配,即时过滤。"),
            (self.btn_open, "④ 管理",
             "选中表可「重新分类」或「移除」;点「打开归档目录」在资源管理器里直接看文件。\n"
             "归档的是副本,增删不影响你其他位置的原文件。"),
        ]
