...
class ReconcilePage(BasePage):
    ...
    def guide_steps(self):
        return [
            (None, "欢迎使用工时对账",
             "这个页面把「数据来源」与「对账单」逐项核对,填进待对表,\n"
             "输出异常汇总并给一个可信度评分。跟着高亮走一遍即可上手。"),
            (self.z_tgt, "① 放待对表(目标 · 单个)",
             "需要被核对填写的总表,选 1 个。\n"
             "程序在这张表上核对并标注,另存为「_已填写」,不动原件。"),
            (self.z_src, "② 放数据来源",
             "已填好的考勤/工时数据,可多选。可由「考勤填报」直接送过来。\n"
             "这是核对的事实来源:每人每天的实际工时。"),
            (self.z_labor, "③ 放对账单 / 工时单",
             "需要与来源核对的对账单据,可多选。\n"
             "与数据来源逐项比对,对不上的差异会计入异常汇总。"),
            (self.panel, "开始对账 · 看可信度",
             "直接点「开始对账」即可;想先核对列映射/姓名配对,点「人工确认后对账…」在右侧确认。\n"
             "完成后状态行给出【可信/需复核/存疑】评分与异常条数,点「打开输出文件夹」看结果。"),
        ]
