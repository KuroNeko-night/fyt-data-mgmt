...
class AttendancePage(BasePage):
    ...
    def guide_steps(self):
        return [
            (None, "欢迎使用考勤数据填报",
             "这个页面把打卡系统导出的原始数据,自动填进待填的考勤表并算好工时。\n"
             "跟着高亮走一遍,约 30 秒上手。点「下一步」开始。"),
            (self.z_src, "① 放系统数据(打卡来源)",
             "把考勤机/系统导出的原始打卡表拖到这里,可多选。\n"
             "这是数据来源:程序从中读取每人每天的打卡时间。"),
            (self.z_tgt, "② 放待填考勤表(目标)",
             "再放需要被填写的考勤表模板,可多选。\n"
             "程序把打卡与工时写进这些表,另存为「_已填写」,不动原件。"),
            (self.panel, "开始填报 · 看结果",
             "点「开始填报」,会先在右侧弹出确认计算口径的面板,确认后开始。\n"
             "完成后可点「打开输出文件夹」查看,或用「把结果送去工时对账」直接接力下一步。"),
        ]
